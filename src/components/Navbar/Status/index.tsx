...
import { EQUIPMENT_SLOTS, SLOT_LABELS, RANK_COLORS } from "@/utils/types/player/equipment";
      ...
      <div className={styles.flexRow}>
        ...
        <div className={styles.flexColumn}>
          <div className={selectedIndex === 0 ? "active" : ""}>
            <p>Vida: {stats.hp}{bonus.hp > 0 ? <span style={{ color: "#4ade80" }}> +{bonus.hp}</span> : ""}</p>
          </div>

          <div className={selectedIndex === 1 ? "active" : ""}>
            <p>Força: {stats.strength}{bonus.strength > 0 ? <span style={{ color: "#4ade80" }}> +{bonus.strength}</span> : ""}</p>
          </div>

          <div className={selectedIndex === 2 ? "active" : ""}>
            <p>Inteligência: {stats.intelligence}{bonus.intelligence > 0 ? <span style={{ color: "#4ade80" }}> +{bonus.intelligence}</span> : ""}</p>
          </div>
            ...
            {(EQUIPMENT_SLOTS as EquipmentSlot[]).map((slot) => {
              ...
              return (
                <p key={slot} style={{ fontSize: 12, marginTop: 4 }}>
                  {label}: {item ? <span style={{ color: RANK_COLORS[item.rank] }}>{item.name}</span> : <span style={{ color: "#666", fontStyle: "italic" }}>Vazio</span>}
                </p>
              );
            })}
          </div>
